...
class GalleryEditItem extends Component {
  ...
  editItem(e, index) {
    ...
    const tagNodes = document.querySelectorAll(
      ".GalleryEditItem__modal--form--tags--checkbox"
    );
    let tags = [];
    ...
    axios
      .post("http://localhost:1337/api/gallery/edit", data)
      .then(res => {
        this.props.editItem(null);
        this.props.toast("success", "Successfully edited item: " + description);
      })
      .catch(err => {
        this.props.toast("error", "Error editing item, please try again later");
        console.log(err);
      });
  }
  ...
  resetTags = () => {
    ...
    tagNodes.forEach(tag => {
      ...
      const checked = this.props.imgData.tags.includes(tag.innerHTML)
        ? true
        : false;
      if (checked) {
  ...
  render() {
    ...
    return (
      ...
      >
        <button
          className="GalleryEditItem__close"
        ...
        <div className="GalleryEditItem__modal">
          <img
            src={"/images/gallery/full-size/" + imgData.slug + ".jpg"}
            className={
              "GalleryEditItem__modal--image " + this.state.imageStatus
            }
          ...
          <div className="GalleryEditItem__modal--form">
            ...
            <form onSubmit={e => this.editItem(e, index)}>
              ...
              <div className="GalleryEditItem__modal--form--tags">{Tags}</div>
